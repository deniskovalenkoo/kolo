...
## Two i18n approaches — pick one consciously per project

A Webflow site you're migrating may have several languages. There are
**two fundamentally different** ways to model i18n in the new Astro
project, and the choice has long-term consequences:

### A. Mirror i18n (what kolo uses today)
Each language is a **completely separate scraped HTML file**. Every page
template exists 1:1 per locale: `src/pages/buy.astro`, `src/pages/ua/buy.astro`,
`src/pages/ru/buy.astro` — all three independent files generated by the
scraper.

**Pros:** zero risk of losing translations from the source, fast to set up
(scraper does it), languages can diverge freely.
**Cons:** changing the hero text means editing it 3× (once per locale);
no shared layout means a CSS fix has to be applied per file; adding a
new language means scraping or hand-translating 38 templates.

**Use for:** Phase 1 mirror migrations where you need to ship fast and
can't afford to lose anything from the source.

### B. Native i18n (what eResidency uses)
**One template per page**, with translations stored in JSON files keyed
by language. Standard Astro pattern using dynamic routes.

```
src/
  pages/[lang]/
    index.astro          ← single file, drives ru/en/uk/zh/ar
    buy.astro
    quiz/start.astro
  i18n/
    ru.json              ← {"hero.title": "Крипто Visa Карта"}
    en.json              ← {"hero.title": "Crypto Visa Card"}
    uk.json
    zh.json
    ar.json
```
...
**Pros:** one template means one source of truth — fix the layout once,
fix it everywhere; new language = one new JSON file; can wire to Sanity
(multi-locale fields) for non-tech editor workflow; impossible for
languages to drift apart accidentally.
**Cons:** writing translations is upfront work (or scrape + AI-translate);
you pay the cost of restructuring the project before it pays back.

**Use for:** new pages added to a mirror project (programmatic SEO, blog
post templates, landing pages); brand-new sites built from scratch in
Astro; sites where editor velocity matters more than initial migration speed.

### Mixing both (the realistic path)
Most real projects end up hybrid: legacy pages stay as Mirror, new pages
are Native, and over time pages get rewritten one-by-one when they need
significant changes. Don't try to convert everything at once — that's
2-4 days of work for a 38-template project with no immediate visible
upside. Convert when you have a reason to touch the page anyway.
